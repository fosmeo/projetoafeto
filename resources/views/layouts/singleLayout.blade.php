...
<head>
    ...
    <meta property="og:title" content="{{ $post -> posts_titulo }}" />
    <meta property="og:description" content="{{ substr($post -> posts_texto, 0, 255) }}... " />

    @if(!empty($post -> posts_imagem))
      <meta property="og:image" content="{{ asset('imagens/imagens_posts/'.$post -> posts_imagem) }}" />
    @elseif(!empty($post -> posts_video))
      {!! preg_match_all('@((https://)[a-zA-Z0-9./?&_\-#=;%]+)@', $post -> posts_video, $match) !!}
      <meta property="og:video" content="{!! $match[0][0] !!}" />
    @else
